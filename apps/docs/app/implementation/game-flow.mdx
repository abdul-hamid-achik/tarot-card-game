...
The game uses XState to manage the complete game flow with proper state transitions. The FSM lives in the shared engine at `packages/game-sim/src/game-flow.ts` and is imported in the web app from `@tarot/game-sim`.
...
npx xstate visualize packages/game-sim/src/game-flow.ts
```
