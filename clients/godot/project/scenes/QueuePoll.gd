...
func _on_press() -> void:
	http.request("http://localhost:3000/api/match/queue", ["Content-Type: application/json"], HTTPClient.METHOD_POST, "{\"userId\":\"GODOT\"}")
...
func _on_queue_done(result: int, code: int, headers: PackedStringArray, body: PackedByteArray) -> void:
	...
	req.request("http://localhost:3000/api/match/result?userId=GODOT")
